...
def user_based_collaborative_filtering(user_id, k=5, top_n=10):
    ...
    for movie_idx in range(len(movies)):
        if user_movie_matrix[target_user_idx, movie_idx] == 0:
            numerator = sum(
                user_similarity[target_user_idx, user_idx]
                * user_movie_matrix[user_idx, movie_idx]
                for user_idx in similar_users
                if user_movie_matrix[user_idx, movie_idx] != 0
            )
            denominator = sum(
                user_similarity[target_user_idx, user_idx] for user_idx in similar_users
            )
