...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        actors = [
            Actor(name=f"Actor {i}", surname=f"Актер {i} Фамилия")
        ...
        for user in all_users:
            ...
            preferred_movies = Movie.objects.filter(
            ...
            for movie in preferred_sample + other_sample:
                rating = random.choices(
                    [None, round(random.uniform(3.0, 5.0), 1)], weights=[0.3, 0.7]
                )[0]
        ...
        for movie in movies_to_update:
            movie.average_rating = (
                Interaction.objects.filter(movie=movie).aggregate(Avg("rating"))[
                    "rating__avg"
                ]
                or 0.0
            )
